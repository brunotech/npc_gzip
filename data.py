...
def _load_csv_filepath(csv_filepath: str) -> list:
    ...
    with open(csv_filepath, "r") as file:
        ...
        data.extend([row[0], row[1], row[2]] for row in reader)
    return data
...
def read_label(filename: str) -> list:
    ...
    return [row[0] for row in _load_csv_filepath(filename)]
...
def load_kinnews_kirnews(
    ...
    def process(dataset: Iterable) -> list:
        ...
        for pair in dataset:
            ...
            pairs.append((label, f"{title} {content}"))
        return pairs
...
def pick_n_sample_from_each_class(
    ...
    class2count = {class_: len(label2text[class_]) for class_ in label2text}
    ...
    return recorded_idx if idx_only else (result, labels)
...
def pick_n_sample_from_each_class_given_dataset(
    ...
    class2count = {cl: len(label2text[cl]) for cl in label2text}
    for c in class2count:
        select_idx = np.random.choice(class2count[c], size=n_samples, replace=False)
        select_text = np.array(label2text[c])[select_idx]
        select_text_idx = np.array(label2idx[c])[select_idx]
    ...
    return (np.array(recorded_idx), labels) if index_only else (result, labels)
...
def pick_n_sample_from_each_class_img(
    ...
    class2count = {cl: len(label2img[cl]) for cl in label2img}
    for c in class2count:
        select_idx = np.random.choice(class2count[c], size=n_samples, replace=False)
